...
function CollectionPanel(props) {
  ...
  if (data.response === null || data === undefined) {
    collection = [
      {
        id: '',
        name: '',
    ...
    console.log('Collection NOW');
    console.log(collection);
  } else {
    collection = data.response.locations;
  }
  ...
  const [selectedIndex, setselectedIndex] = useState(0); //useState for triggering data fetching

  function handleCollectionSelection(props, indexNum) {
    setSelectedMarkers(props.spots);

    console.log('indexNum handle' + indexNum);
...
export async function getServerSideProps(context) {
  ...
  const res = await fetch(
    `http://95.217.16.239:4000/api/collection?user=testi5@testi5.com`, //user=testi5@testi5.com antaa dataa
  );
